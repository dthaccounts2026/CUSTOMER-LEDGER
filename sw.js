...
// Bump this version string whenever customer-ledger.html changes so the
// old cached copy is thrown away and the new one takes over.
const CACHE_NAME = 'ledgerbooks-v1';
const APP_SHELL = [
...
self.addEventListener('fetch', (event) => {
  ...
  const isAppHtml = req.mode === 'navigate' || req.url.includes('customer-ledger.html');
  ...
  if (isAppHtml) {
    event.respondWith(
      fetch(req)
        ...
        .catch(() => caches.match(req).then((res) => res || caches.match('./customer-ledger.html')))
    );
    return;
  }
